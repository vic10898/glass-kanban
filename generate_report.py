...
def create_report():
    ...
    add_p(
        "Данный проект — это веб-приложение для управления задачами (канбан-доска). "
        "Интерфейс оформлен в темной теме с эффектом матового стекла (Glassmorphism). "
        "Приложение позволяет пользователям создавать персональные доски проектов, добавлять колонки состояний "
        "(например: «В планах», «В работе», «Готово») и управлять карточками задач. Реализована поддержка "
        "drag-and-drop для перемещения задач между колонками, настройка приоритета и срока выполнения, а также "
    ...
    add_p(
        "В процессе прохождения производственной практики была выполнена задача по проектированию "
        "и разработке веб-приложения для управления задачами «Glass Kanban». "
    ...
    add_p(
        ...
        "3. Разработка интерфейса. Интерфейс стилизован под Glassmorphism (эффект матового стекла), реализована темная тема. Матовые полупрозрачные блоки "
        "выглядят современно и не перегружают экран.\n"
        "4. Интеграция Drag-and-Drop. Чтобы не перегружать проект тяжелыми библиотеками, перетаскивание реализовано на чистом "
        "HTML5 Drag and Drop API. Настроены обработчики перетаскивания и моментальное визуальное обновление интерфейса, "
        "которое сразу сдвигает карточку на экране, а затем сохраняет новое состояние в базу данных.",
        space_after=12
    )

    add_p(
        "Среди основных трудностей, возникших при реализации проекта, можно выделить:\n"
        "• Настройка middleware в Next.js для проверки авторизации. Поскольку "
        "библиотека `jsonwebtoken` не работает в Edge Middleware, пришлось разобраться, как правильно читать cookies на стороне сервера, "
        "чтобы закрыть доступ к доскам для неавторизованных пользователей и выполнять проверку токена уже в API-маршрутах.\n"
        "• Реализация перетаскивания карточек. Нужно было правильно обрабатывать "
        "события dragstart, dragover и drop, чтобы карточки перемещались между колонками без багов. "
        "Написание собственной логики перетаскивания позволило не использовать тяжелые библиотеки.",
        space_after=12
    )
